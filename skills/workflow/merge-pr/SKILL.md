...
- Common layout: `~/.config/sessions/<topic>/worktree-<repo>/` (via `node $CLAUDE_SKILL_DIR/session-topic.mjs worktree-path <topic> [dir]`; `<repo>` is auto-derived from the git main worktree basename)
