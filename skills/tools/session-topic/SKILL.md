...
**First spec:** `init "<hint>"` (reuse the conversation's topic) → `artifact-create <topic> spec <name>` → `gco-latest` any repo whose code you read → follow `references/spec-writing-guide.md`, pass the Determinism Gate → **stop and confirm**; never auto-create the plan in the same pass.
